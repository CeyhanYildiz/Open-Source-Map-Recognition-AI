...
with col_image:
    ...
    # Placeholder for image display + predictions
    img_display = st.empty()

with col_models:
    st.subheader("Predictions")
    cols = st.columns(2)
    placeholder_A = cols[0].container()
    placeholder_B = cols[1].container()
...
def render_predictions(img: Image.Image):
    ...
    # show image
    img_display.image(display_img, use_column_width=True)
